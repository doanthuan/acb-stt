...
def test_a_call(call):
  ...
  cursor.execute(f"SELECT text_content FROM stt_call_conversation WHERE call_id={call['call_id']} AND line='customer' ")
  ...
  current_text = {"names": "", "addresses": "", "id": "", "phone": "", "card_no":"", "acc_no":""}
  for text in texts:
...
for row in rows:
    calls.append({
      "call_id":row[0],
      "audio": os.path.basename(str(row[1]))
      })

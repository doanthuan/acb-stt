...
def save_audio(path: str, tensor: torch.Tensor, sr: int = 16000):
...
def get_speech_ts_adaptive(
    ...
    trig_sum = (
        0.89 * median_probs + 0.08
    )  # 0.08 when median is zero, 0.97 when median is 1
        ...
        if i != len(speeches) - 1:
            ...
            if silence_duration < 2 * speech_pad_samples:
                ...
                speeches[i + 1]["start"] = max(
                    0, speeches[i + 1]["start"] - silence_duration // 2
                )
            else:
...
def get_language(wav: torch.Tensor, model, run_function=validate):
    wav = torch.unsqueeze(wav, dim=0)
...
class VADiterator:
    ...
    def prepare_batch(self, wav_chunk, name=None):
        ...
        if len(wav_chunk) < self.num_samples:
            wav_chunk = F.pad(
                wav_chunk, (0, self.num_samples - len(wav_chunk))
            )  # short chunk => eof audio
            self.last = True
        ...
        overlap_chunks = [
            stacked[i : i + self.num_samples].unsqueeze(0)
    ...
    def state(self, model_out):
        ...
        for i, predict in enumerate(speech_probs):
            ...
            if (
                (sum(self.buffer) / len(self.buffer)) >= self.trig_sum
            ) and not self.triggered:
                self.triggered = True
                current_speech[
                    self.num_frames - (self.num_steps - i) * self.step
                ] = "start"
            if (
                (sum(self.buffer) / len(self.buffer)) < self.neg_trig_sum
            ) and self.triggered:
                current_speech[
                    self.num_frames - (self.num_steps - i) * self.step
                ] = "end"
...
class VADiteratorAdaptive:
    ...
    def state(self, model_out):
        current_speech = {}
        speech_probs = model_out[
            :, 1
        ]  # 0 index for silence probs, 1 index for speech probs
        for i, predict in enumerate(speech_probs):
...
def state_generator(
    ...
    VADiters = [
        VADiterator(trig_sum, neg_trig_sum, num_steps, num_samples_per_window)
        for i in range(audios_in_stream)
    ]
    for i, current_pieces in enumerate(
        stream_imitator(audios, audios_in_stream, num_samples_per_window)
    ):
...
def stream_imitator(
    audios: List[str], audios_in_stream: int, num_samples_per_window: int = 4000
):
...
def single_audio_stream(
    ...
    **kwargs
    ...
    if iterator_type == "basic":
        ...
        VADiter = VADiteratorAdaptive(
            num_samples_per_window=num_samples_per_window, **kwargs
        )

    wav = read_audio(audio)
    wav_chunks = iter(
        [wav[i : i + num_samples] for i in range(0, len(wav), num_samples)]
    )
